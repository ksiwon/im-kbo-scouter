...
export const generateContextNote = (sectionId: string): string => {
  switch (sectionId) {
    ...
    case 'failure':
      return '지미 파레디스, 루크 스캇, 모터, 그리고 수많은 "거포들"의 실패. 그들의 공통점은 무엇이었을까요? AAA에서 30홈런을 쳤던 타자도 KBO에서는 2할 초반대에 그칠 수 있습니다.';
    ...
    case 'distribution':
      return '타율, 출루율 같은 클래식 지표는 성공을 장담하지 못합니다. Box Plot을 주목하세요. 성공 그룹은 안정적인 분포를 보이지만, 실패 그룹(Worst)은 선수별 편차(Variance)가 극심하여 예측이 불가능한 패턴을 보입니다.';
